"""
File system handling tools:
 - reading a file,
 - listing files in a repo (also recursively)
 - finding file by a pattern
 - find files where a certain string pattern is present
 - replace file contents with another file content
"""
import fnmatch
...
def read_file(file_path: str) -> str:
    """
    This tool reads and returns the contents of a file.
...
def list_files(directory: str, recursive: bool = False) -> str:
    """
    This tool lists files in a directory, optionally recursively.

    Args:
        directory (str): absolute path to the directory to list
        recursive (bool): if True, list files in all subdirectories as well
...
def find_file(directory: str, pattern: str) -> str:
    """
    This tool finds files matching a glob pattern within a directory.

    Args:
        directory (str): absolute path to the directory to search
        pattern (str): glob pattern to match filenames against (e.g., '*.py', 'config*.json')
...
def grep_files(directory: str, substrings: list[str], recursive: bool = True) -> str:
    """
    This tool finds files whose contents contain all of the given substrings.

    Args:
        directory (str): absolute path to the directory to search
        substrings (list[str]): one or more strings that must all be present in a file
        recursive (bool): if True, search files in all subdirectories as well

    Returns:
        str: newline-separated list of matching file paths with the matching lines
    """
    path = Path(directory).expanduser()
    if not path.exists():
        return f"Error: Directory not found: {directory}"
...
def replace_file_contents(source_file: str, destination_file: str) -> str:
    """
    This tool overwrites the contents of destination_file with the contents of source_file.

    Args:
        source_file (str): absolute path to the file to copy from
        destination_file (str): absolute path to the file to overwrite
